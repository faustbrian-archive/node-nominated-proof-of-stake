...
it("should create a list of validators that can mine", () => {
  const result = round({
    ...
    round: 1,
    validators: [
      validator(publicKeys[0]!, [{ stake: 1 }]),
      validator(publicKeys[1]!, [{ stake: 2 }]),
      validator(publicKeys[2]!, [{ stake: 3 }]),
      validator(publicKeys[3]!, [{ stake: 4 }]),
      validator(publicKeys[4]!, [{ stake: 5 }]),
      validator(publicKeys[5]!, [{ stake: 0 }]),
      validator(publicKeys[6]!, [{ stake: 0 }]),
      validator(publicKeys[7]!, [{ stake: 0 }]),
      validator(publicKeys[8]!, [{ stake: 0 }]),
      validator(publicKeys[9]!, [{ stake: 0 }]),
    ],
  });
